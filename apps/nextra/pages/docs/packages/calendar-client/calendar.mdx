...
import { DefaultCalendar,AdvancedCalendar } from "../../../../example/calendar-client";
...
Calendar is using following (i18next) translations from namespace **`common`**
- `calendar.today.icon.tooltip`: tooltip over selecting current date icon
- `calendar.search.icon.tooltip`: tooltip over switching calendar to "search mode"
- `calendar.with-weeks.icon.tooltip`: tooltip about switching number of weeks
