...
export const use${name}Mutation = createMutationHook<string[], InferSource.Item<${source}>, ${queryParams}>(${name}ApiLink, "post");

export const to${name}Link = (queryParams?: ${queryParams}) => toLink(${name}ApiLink, queryParams);
export const use${name}Link = () => to${name}Link;

export const use${name}Promise = createPromiseHook<string[], InferSource.Item<${source}>, ${queryParams}>(${name}ApiLink, "post");

export const ${name}Promise = createPromise<string[], InferSource.Item<${source}>, ${queryParams}>(${name}ApiLink, "post");
`);
}
